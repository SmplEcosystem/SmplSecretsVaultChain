package keeper_test

import (
	"strconv"
	"testing"

	keepertest "github.com/SmplEcosystem/SmplSecretsVaultChain/testutil/keeper"
	"github.com/SmplEcosystem/SmplSecretsVaultChain/testutil/nullify"
	"github.com/SmplEcosystem/SmplSecretsVaultChain/x/smplsecretsvaultchain/keeper"
	"github.com/SmplEcosystem/SmplSecretsVaultChain/x/smplsecretsvaultchain/types"
	sdk "github.com/cosmos/cosmos-sdk/types"
	"github.com/stretchr/testify/require"
)

// Prevent strconv unused error
var _ = strconv.IntSize

func createNWallets(keeper *keeper.Keeper, ctx sdk.Context, n int) []types.Wallets {
	items := make([]types.Wallets, n)
	for i := range items {
		items[i].Owner = strconv.Itoa(i)

		keeper.SetWallets(ctx, items[i])
	}
	return items
}

func TestWalletsGet(t *testing.T) {
	keeper, ctx := keepertest.SmplsecretsvaultchainKeeper(t)
	items := createNWallets(keeper, ctx, 10)
	for _, item := range items {
		rst, found := keeper.GetWallets(ctx,
			item.Owner,
		)
		require.True(t, found)
		require.Equal(t,
			nullify.Fill(&item),
			nullify.Fill(&rst),
		)
	}
}
func TestWalletsRemove(t *testing.T) {
	keeper, ctx := keepertest.SmplsecretsvaultchainKeeper(t)
	items := createNWallets(keeper, ctx, 10)
	for _, item := range items {
		keeper.RemoveWallets(ctx,
			item.Owner,
		)
		_, found := keeper.GetWallets(ctx,
			item.Owner,
		)
		require.False(t, found)
	}
}

func TestWalletsGetAll(t *testing.T) {
	keeper, ctx := keepertest.SmplsecretsvaultchainKeeper(t)
	items := createNWallets(keeper, ctx, 10)
	require.ElementsMatch(t,
		nullify.Fill(items),
		nullify.Fill(keeper.GetAllWallets(ctx)),
	)
}
